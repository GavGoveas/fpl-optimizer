class ChipsOptimizer:
    def __init__(self, players=None, bench=None, free_hit_gain=0.0, wildcard_gain=0.0, available=None):
        self.players = list(players or [])
        self.bench = list(bench or [])
        self.free_hit_gain = float(free_hit_gain)
        self.wildcard_gain = float(wildcard_gain)
        self.available = set(available or {"TC", "BB", "FH", "WC"})
    ...
    def recommend_chip_usage(self):
        ...
        return {"best_chip": best_chip, "opportunities": eligible, "recommendation": eligible.get(best_chip)}
